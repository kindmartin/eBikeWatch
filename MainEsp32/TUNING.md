...
- `t.pr_offload_sleep(action="arm", wake="main_rx_mirror", delay_ms=750)` &rarr; arm the new deep/light sleep path using the existing UART wiring (GPIO33 tied to the master TX line through a 47–100&nbsp;kΩ resistor). Use `action="cancel"` to abort before the countdown elapses; wake the offload by driving the mirrored line low (e.g., transmit a burst of `0x00`).
