...
t.enable_control_loop_debug(1, period_ms=2000)

# show loop timing breakdown (ADC / CPU / PID buckets)
t.set_loop_timing_monitor(1)

# focused PID math timing dump
...
### Disabling Phaserunner Telemetry for Speed-Only Tests

Speed mode does not require Phaserunner data. To remove those background reads before you capture loop `dt` logs, run the following sequence in the REPL:

```python
import t

# 1) Make sure the verbose monitor is off (avoids interleaved prints)
t.enable_control_loop_debug(0)

# 2) Disable the Phaserunner thread; stays off until reboot or set True
t.set_pr_thread(False)

# 3) Bring the monitor back exactly as before, but now PR is silent
t.enable_control_loop_debug(1, period_ms=500)
t.set_loop_timing_monitor(1)
t.set_pid_timing_debug(1, period_ms=500)
```

You now get the same monitor + `dt` breakdowns with the Phaserunner worker fully disabled. Re-enable it later with `t.set_pr_thread(True)` if you need live PR stats again.


>>> t.enable_control_loop_debug(0)
[t] control loop debug OFF
False
>>> t.set_loop_timing_monitor(1)
[t] loop timing monitor ON
True
>>> t.set_pid_timing_debug(1)
[t] pid timing debug ON (1000 ms)
>>> t.enable_control_loop_debug(1, period_ms=500)
[t] control loop debug ON (500 ms)
...
1. `t.enable_control_loop_debug(1, period_ms=500)` – start the main monitor loop.
2. `t.set_loop_timing_monitor(1)` – append `timing[...]` with ADC/CPU/DAC plus section breakdown (sensors/controller/post/other).
3. `t.set_pid_timing_debug(1, period_ms=500)` – prints an extra `pid-debug` line after each monitor row summarizing the P/I/D math (`err`, `int`, `der`, `dt`, `out`, `pid=Xms`).
4. Exercise the throttle or force a target. When the loop slows, inspect `timing[...]`: if `ctrl` spikes, read the `pid-debug` line to see whether the speed PID (`speed:` block) is consuming hundreds of ms. If `spd` or `pow` entries swell, the telemetry fetch is the offender.
5. Turn everything off again with `t.enable_control_loop_debug(0)`, `t.set_loop_timing_monitor(0)`, `t.set_pid_timing_debug(0)` once you capture the data.

This sequence keeps ADC/DAC logging separate from the PID math timing while showing both when needed.
...
# Force the speed loop to chase a fixed km/h regardless of throttle
t.set_speed_target(35)   # hold 35 km/h
t.set_speed_target(None) # release back to ADC throttle
```
