...
async def trip_counter_task(
    ...
    *,
    ...
    resume_pending = bool(getattr(state, "trip_resume_pending", False))
    ...
    try:
        while True:
            ...
            offset = getattr(state, "trip_pulse_offset", 0)
            try:
                offset = int(offset)
            except Exception:
                offset = 0
            total_pulses = pulses + offset
            state.trip_pulses = total_pulses
            distance_m = total_pulses * pulse_to_meter
            state.trip_distance_m = distance_m
            state.trip_distance_km = distance_m / 1000.0 if pulse_to_meter else 0.0
            if getattr(state, "trip_resume_pending", False):
                state.trip_resume_pending = False
...
async def gc_task(config_source):
    """Periodic garbage collector driver."""
    ...
    try:
        while True:
            ...
            try:
                interval_cfg = int(cfg.get("interval_ms", 60000))
            except Exception:
                interval_cfg = 60000
            if interval_cfg < minimum:
                interval_cfg = minimum

            before = mem_free_fn() if callable(mem_free_fn) else None
            after = None
            if enabled:
                try:
                    gc.collect()
                    ...
                    if verbose:
                        print("[GC] collect error:", exc)
                finally:
                    after = mem_free_fn() if callable(mem_free_fn) else None
                    if verbose:
                        if before is not None and after is not None:
                            delta = after - before
                            print("[GC] free {} -> {} (delta {})".format(before, after, delta))
                        else:
                            print("[GC] collect done")
            elif verbose:
                print("[GC] skipped (disabled)")
